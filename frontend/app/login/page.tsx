      ...
      if (!response.ok) {
        ...
        const errorMessage =
          typeof body?.detail === "string"
            ? body.detail
            : body?.detail?.message ?? "Invalid credentials.";
        throw new Error(errorMessage);
      }

      const data = (await response.json()) as { access_token: string; token_type: string };
      document.cookie = `ats_auth_token=${data.access_token}; path=/; max-age=86400`;
    ...
    <div className="relative flex min-h-[calc(100vh-4rem)] items-center justify-center overflow-hidden px-4 py-10">
      ...
      >
        <div className="text-center">
          <p className="text-sm font-medium text-indigo-400">Enterprise ATS</p>
        ...
        <form className="mt-8 space-y-4" onSubmit={handleLogin}>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full rounded-lg border-none bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white shadow-[0_0_15px_rgba(79,70,229,0.5)] transition-all duration-300 hover:bg-indigo-500 disabled:cursor-not-allowed disabled:bg-indigo-400"
          >
